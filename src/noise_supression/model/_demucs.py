...
class Demucs(nn.Module):
    ...
    def __init__(
        ...
        for index in range(depth):
            ...
            decode += [
                ...
                activation,
                nn.ConvTranspose1d(
                    hidden, chout, kernel_size, stride
                ),
            ]
            if index > 0:
    ...
    def forward(self, mix):
        ...
        if self.resample == 2:
            ...
            self.resample == 4
        ):  # https://arxiv.org/pdf/2006.12847.pdf / The resampling is done using a sinc interpolation filter
            x = upsample2(x)
            x = upsample2(x)
        skips = []
...
def fast_conv(conv, x):
    ...
    if kernel == 1:
        ...
        out = th.addmm(
            conv.bias.view(-1, 1), conv.weight.view(chout, chin), x
        )
    elif length == kernel:
        x = x.view(chin * kernel, 1)
        out = th.addmm(
            conv.bias.view(-1, 1),
            conv.weight.view(chout, chin * kernel),
            x,
        )
    else:
        out = conv(x)
...
class DemucsStreamer:
    ...
    def __init__(
        ...
        self.frame_length = demucs.valid_length(
            1
        ) + demucs.total_stride * (num_frames - 1)
        self.total_length = (
            self.frame_length + self.resample_lookahead
        )
        self.stride = demucs.total_stride * num_frames
        self.resample_in = th.zeros(
    ...
    def feed(self, wav):
        ...
        if chin != demucs.chin:
            raise ValueError(
                f"Expected {demucs.chin} channels, got {chin}"
            )
        ...
        while self.pending.shape[1] >= self.total_length:
            ...
            if demucs.normalize:
                ...
                self.variance = (
                    variance / self.frames
                    + (1 - 1 / self.frames) * self.variance
                )
                frame = frame / (
                    demucs.floor + math.sqrt(self.variance)
                )
            frame = th.cat([self.resample_in, frame], dim=-1)
            self.resample_in[:] = frame[
                :, stride - resample_buffer : stride
            ]
